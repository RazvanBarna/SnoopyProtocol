...
Fetch: IFetch0 port map(clk=>clk, en =>en, rst=>btnRst, jump=>jump, PcSrc=>PcSrc,branchAddress=>branchAddress,jumpAddress=>jumpAddress,
...
MemComp: Mem0 port map (clk=>clk,useCC => useCC,line_debug => line_debug,wb =>wb,readWriteCC=> readWriteCC_aux,data_fromCC => data_fromCC,send_data_to_bus=>send_data_to_bus_aux,RD2=>RD2,aluResOut=>aluResOut,aluRes=>aluResAux,en=>en,memWrite=>memWrite,memData=>memData);
