...
signal m : matrix := (
    ...
    0 => "0000" &X"9877" & "00" & "000100" & "0000" & X"0BBBBBBB", --  +5   (pozitiv) stare tag index offset data
...
find_line: process(clk) -- caut daca nu e 0 ceea ce primesc
        ...
        if rising_edge(clk) then
            ...
            if m(i) = data_fromCC and (not (data_fromCC = X"0000000000000000")) then
                line_indexCc <= i;
                exit;
            end if;
        end loop;
...
begin   
    if rising_edge(clk) then 
        ...
        elsif en='1' and memWrite='1' and lw_swInstr = '1' then -- mereu cand scrie trimit cerere
            ucc_aux<= '1' ;
