...
signal en,lw_swInstr_aux,jump,PcSrc,regWrite,readWriteCC_aux,regDst,extOp,aluSrc,zero,memWrite,memToReg,branch,bgtz,bne,gtz: std_logic :='0';
...
Fetch: IFetch1 port map(clk=>clk,next_instr_core1 => next_instr_core1, en =>en, rst=>btnRst, jump=>jump, PcSrc=>PcSrc,branchAddress=>branchAddress,jumpAddress=>jumpAddress,
...
MemComp: Mem1 port map (clk=>clk,lw_swInstr => lw_swInstr_aux,useCC => useCC,line_debug => line_debug,wb =>wb,readWriteCC=> readWriteCC_aux,data_fromCC => data_fromCC,send_data_to_bus=>send_data_to_bus_aux,RD2=>RD2,aluResOut=>aluResOut,aluRes=>aluResAux,en=>en,memWrite=>memWrite,memData=>memData);
